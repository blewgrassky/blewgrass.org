...
export const renderHeroSection = prefix => {
	...
	if (
		getOptionFor('has_category_label', prefix) &&
		getOptionFor('has_category_label', prefix) !== 'yes'
	) {
		;[
			...document.querySelectorAll('.hero-section .page-title span')
		].map(el => el.remove())
	}
